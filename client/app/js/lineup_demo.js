...
(function (LineUpJS) {
    ...
    function updateMenu() {
        ...
        d3.select('#lugui-menu-actions').selectAll('span').data(menuActions).enter().append('span').html(function (d) {
    ...
    function initLineup(name, desc, _data) {
        ...
        cols.forEach(function (rankCol) {
            rankCol.children.forEach(function (col) {
                if (col.desc.type === "stack") col.sortByMe();
            })
        })
        updateMenu();
    }
    ...
    function saveToGist() {
        ...
        d3.json('https://api.github.com/gists').post(JSON.stringify(args), function (error, data) {
            if (error) {
                ...
                history.pushState({
                    id: 'gist:' + id
                }, 'LineUp - ' + (args.description || 'Custom'), '#gist:' + id);
            }
        });
    }
    ...
    // document ready
    d3.json('../data/datasets.json', function (error, data) {
        ...
        ds.enter().append('option').attr('value', function (d, i) {
            return i;
        }).text(function (d) {
            return d.name;
        });
